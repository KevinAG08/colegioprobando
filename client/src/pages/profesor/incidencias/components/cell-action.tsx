...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const { mutate: deleteIncidencia, isPending } = useDeleteIncidencia("profesor");

  const onEdit = () => {
    navigate(`/profesor/incidencias/editar/${data.id}`);
  };

  const onSee = () => {
    navigate(`/profesor/incidencias/ver/${data.id}`);
  };
